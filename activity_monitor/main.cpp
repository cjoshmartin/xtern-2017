...
int main()
{
  string input; // where we will be storing our input for the short while
  ...
  while(getline(cin, input)) // will store the the current line from user input into `input` string
  {
    ...
    // if true make entry into map && sets : else will naggates the value at that postion in the map
  ...
  for(map<string, bool >::iterator it = processes.begin(); it != processes.end(); ++it) { // iterating through the map
    if(processes[it->first]) // check to see if a process is running
