...
int main()
{
  ...
  while(getline(cin, input)) // takes an input until we don't have anymore newlines
  {
    ...
    {
      // this if statemant is where we check if the path is absolute
      root=input; // stores the absolute path for the relative paths later
      history.insert(history.begin() + i++,root); // push it to the vector so we can travers it later
    }
    else if(input == "BACK")
    {
      i--; // moves back one spot in history
    }
    else if(input == "FORWARD")
    {
      if(i < history.size()) // checking if we can move forward in our history
      i++; // how we move forward
    }
    else if(root!= "") { // making sure that the path has a root path before we start adding relative pathes
      if(input.find("/") == 0) // checks if the path starts with a `/`. EXAMPLE: /tacos
      {
        history.insert(history.begin()  + i++,root + input); // gets the root path and adds on the relative paths then inserts to the current position in history 
      }
      else
      {
        ...
        {
          history.insert( history.begin() + i, history[i-1] + "/" + input); // check if a relative path has been path in before
